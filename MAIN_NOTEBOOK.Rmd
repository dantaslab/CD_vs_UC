...
### anosim analysis 
library(vegan)
whole_proteome <- linear_model_df %>% select(-c(REQUESTED_FILE_NAME, DEIDENTIFIED_MASTER_PATIENT_ID, SAMPLE_COLLECTED_DATE, BIRTH_YEAR, Age, SEX)) 


whole_proteome <- sapply( whole_proteome, as.numeric ) %>% as.data.frame()
anosim(whole_proteome, grouping = whole_proteome$DIAGNOSIS, distance="euclidian", parallel=8, permutations = 999)

subset_proteome <- linear_model_df %>% select(-c(REQUESTED_FILE_NAME, DEIDENTIFIED_MASTER_PATIENT_ID, SAMPLE_COLLECTED_DATE, BIRTH_YEAR)) %>% select(c(Q9Y5Q6, P29460, O95750, P29459_P29460, P16422, P35228, Q99795, P04196, Q02747, P10144, P22455, P09238, Q9Y6Y9, DIAGNOSIS))
subset_proteome <- sapply( subset_proteome, as.numeric ) %>% as.data.frame()
anosim(subset_proteome, grouping = subset_proteome$DIAGNOSIS, distance="euclidian", , parallel=8, permutations = 999)
